...
<title>Entreprises</title>
    ...
    @endif
        <div class="col-lg-12" style="margin-bottom : 30px">
            <div class="pull-left" style="margin-bottom:90px">
            <center> <tr>
                <th><b style="font-size:1.5rem;color:#333333;">Liste </b></th>
                <th><b style="font-size:3rem;color:#333333">Entreprises</b></th>
                <th><b style="font-size:1.5rem;color:#333333">Total : {{ $totalEntreprise }}</b></th>
              </tr><br><br>
            </div>
            ...
            <section class="search-sec" style="margin : 20px ; font-size : 13px">
                <div class="container" style="margin-left : 0">
                    ...
                    <form action="{{ route('entreprises.index') }}" method="get" novalidate="novalidate">
                        <div class="row">
                            <div class="col-lg-40">
                                <div class="row">
                                    ...
                                    <div class="col-lg-2 col-md-2 col-sm-30 p-0">
                                        ...
                                        <button type="button" class="btn btn-danger wrn-btn"><a
                                                href="{{ route('clients.index') }}">Reset</a></button>
                                    </div>
            ...
            <div>
                <p> Il y'a : {{ $totalEntreprise }} Organisations
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="table">
        ...
        <form method="post" id="entreprisesForm">
            @csrf
            @method('DELETE')
            ...
            <a class="btn btn-success" href="{{ route('entreprises.create') }}" style="margin-bottom : 14px "> Ajouter</a>
            <br>
            ...
            <table class="table" style="text-align:center">
                <thead class="thead" style="background:#cccccc">
                    <tr style=" white-space: nowrap;">
                        ...
                        <th>@sortablelink('Num CSE')</th>
                        <th>@sortablelink('Tel')</th>
                        <th>@sortablelink('Email')</th>
                        <th>@sortablelink('Adresse')</th>
                        <th>@sortablelink('Code postal')</th>
                        <th>@sortablelink('ville')</th>
                        <th>@sortablelink('Date Creation')</th>
                        <th>Action</th>
                        <th></th>
                    </tr>
                </thead>
                    ...
                    @foreach ($data as $key => $value)
                        <tr>
                            <td scope="row">
                                <input type="checkbox" name="entreprises[]" id="checks" class="selectbox" value="{{$value->id_organisation}}">
                            </td>
                            <td>{{ $value->id_organisation}}</td>
                            <td> {{ $value->nom}}</td>
                            <td>{{$value->num_cse}}</td>
                            <td>
                                {{$value->tel}}
                            ...
                            <td>{{ $value->ville }}</td>
                            <td> Date Creation à ajouter dans la BDD</td>
                            <td>
                                <a style="background-color : #fff ; border : #fff" class="btn btn-info"
                                    href="{{ route('entreprises.show', ['id_organisation'=>$value->id_organisation]) }}"><i style="color : #4d94ff"
                                        class="fas fa-info"></i></a>
                                <a style="background-color : #fff ; border : #fff" class="btn btn-primary"
                                    href="/entreprises.edit/{{$value->id_organisation}}"><i style="color : #e6ac00"
                                        class="fas fa-edit"></i></a>
                                <button formaction="{{ route('entreprises.destroy',['id_organisation'=>$value->id_organisation]) }}"
        ...
        var checkboxes = document.querySelectorAll('input[name="entreprises[]"]:checked');
        var ids = [];
        ...
        if (ids.length > 0) {
          ...
          form.setAttribute('action', '{{ route("entreprises.deleteselect") }}'); 
          form.submit(); 
        } else {
      ...
      var deleteButton = document.querySelector('#entreprisesForm button[type="submit"]');
