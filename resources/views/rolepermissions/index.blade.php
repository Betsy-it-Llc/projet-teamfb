...
@section('content')
    <div class="row">
        <div class="col-lg-12 margin-tb">
            <div class="pull-left">
                <h2>Gestion des Permissions associées au Role {{ $role_name }}</h2>
            </div>
    ...
    <div class="table" style="margin : 20px">
        ...
        <form method="post">
            @csrf
            @method('DELETE')
            <button type="submit" formaction="{{ route('revokeallpermissions',['id_role'=>$id_role]) }}" class="btn btn-danger" style="margin-bottom : 14px"
                onclick="return confirm('Are you sure you want to revoke all permissions ?')"><i class="fas fa-trash"
                    style="margin-right : 10px"></i><i class="fas fa-arrow-down"></i></button>
            <a class="btn btn-success" href="{{ route('rolepermissions.create',['id_role'=>$id_role]) }}" style="margin-bottom : 14px "> Ajouter</a>
            <a class="btn btn-secondary" href="{{ route('roles.index') }}" style="margin-bottom : 14px ""> Back</a>
            <br><br>
            <p> Il y'a :<?php
            use Illuminate\Support\Facades\DB; 
            $countt = DB::connection('mysql')->table('permissions')
                    ...
                    <tr style="font-size : 12px">
                        ...
                        <td>
                            <a style="background-color : #fff ; border : #fff" class="btn btn-info"
                                href="{{ route('permissions.show', $value->id) }}"><i style="color : #4d94ff"
                                    class="fas fa-info"></i></a>
                            <a style="background-color : #fff ; border : #fff" class="btn btn-primary"
                                href="{{ route('permissions.edit', $value->id) }}"><i style="color : #e6ac00"
                                    class="fas fa-edit"></i></a>
                            <button formaction="{{ route('revokepermission', ['id_role'=>$id_role,'id_permission'=>$value->id]) }}"
                                style="background-color : #fff ; border : #fff" type="submit" class="btn btn-danger"
                                onclick="return confirm('Are you sure you want to revoke this permission ?')"><i style="color : #cc3300"
