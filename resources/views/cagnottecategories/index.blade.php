...
@section('content')
    ...
    <title>Cagnotte Catégorie</title>
    <div class="row">
...
@endif
        <div class="col-lg-12" style="margin-bottom : 30px">
            <div class="pull-left">
            <center> <tr>
                <th><b style="font-size:1.5rem;color:#333333;">Cagnotte </b></th>
                <th><b style="font-size:3rem;color:#333333">Catégorie</b></th>
              </tr><br><br>
                <center>
        ...
        <form method="post" id='clientsForm'>
            ...
            <button type="submit" formaction="/deleteall_g" class="btn btn-danger" style="margin-bottom : 14px"
                onclick="return confirm('Are you sure you want to delete ?')"><i class="fas fa-trash"
                    ...
                    <a class="btn btn-success" href="{{ route('cagnottecategories.create') }}" style="margin-bottom : 14px "> Ajouter</a>
            <br>
            ...
            <table class="table" style="text-align:center">
                <thead class="thead" style="background:#cccccc">
                    <tr style=" white-space: nowrap;">
                        ...
                        <!-- <th>N°</th> -->
                        <th>ID</th>
                        <th>@sortablelink('Nom Catégorie')</th>
                        <th>@sortablelink('Description')</th>
                        <th>@sortablelink('Nrb Contribution')</th>
                        <th>@sortablelink('Montant Total')</th>
                        <th>Action</th>
                        <th></th>
                    </tr>
                </thead>
                    ...
                    <tr>
                        <th scope="row"><input type="checkbox" name="ids[]" class="selectbox"
                                    value="{{$categorie->id_categorie}}"></th>
                            <td> <a href="#"target="_blank">{{$categorie->id_categorie}}</a></td>
                            <td>{{$categorie->nom}}</td>
                            <td>{{$categorie->description}}</td>
                            <td>{{ $contributeursParCategorie[$categorie->id_categorie] }}</td>
                            <td>{{ $montantTotalParCategorie[$categorie->id_categorie] }}</td>


                            <td>
                                <a style="background-color : #fff ; border : #fff" class="btn btn-info"
                                    href="{{ route('cagnottecategories.show', ['id_categorie' => $categorie->id_categorie]) }}
                                    "><i style="color : #4d94ff"
                                        class="fas fa-info"></i></a>
                                        <a style="background-color : #fff ; border : #fff" class="btn btn-primary"
                                        href="{{ route('cagnottecategories.edit', ['id_categorie' => $categorie->id_categorie]) }}">
                                        <i style="color : #e6ac00" class="fas fa-edit"></i>
                                     </a>
                                        <form action="{{ route('cagnottecategories.destroy', $categorie->id_categorie) }}" method="POST">
                                            @csrf
                                            @method('DELETE') <!-- Utilisez la méthode DELETE -->
                                            <!-- Bouton de suppression -->
                                            <button   style="background-color : #fff ; border : #fff" type="submit" class="btn btn-danger" onclick="return confirm('Etes vous sur de vouloir suprimmer cetet catégorie?')">
                                                <i class="fas fa-trash" style="color : #cc3300"></i>
                                            </button>
                                        </form>
                            </td>
                    </tr>
