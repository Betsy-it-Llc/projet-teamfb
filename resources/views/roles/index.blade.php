...
@section('content')
    ...
    <div class="table" style="margin : 20px">
        ...
        <form method="post">
            ...
            <a class="btn btn-success" href="{{ route('roles.create') }}" style="margin-bottom : 14px "> Ajouter</a>
            <br><br>
            <p> Il y'a :<?php
            use Illuminate\Support\Facades\DB; 
            $countt = DB::connection('mysql')->table('roles')
            ->count('roles.id');
            echo $countt;
            ?> roles
            ...
            <table class="table">
                ...
                @foreach ($data as $key => $value)
                    <tr style="font-size : 12px">
                        ...
                        <td>
                            <a style="background-color : #fff ; border : #fff" class="btn btn-info" href="{{ route('rolepermissions.index',['id_role'=>$value->id]) }}"><i style="color : #4d94ff" class="fas fa-eye"></i></a>
                            <a style="background-color : #fff ; border : #fff" class="btn btn-info"
                                href="{{ route('roles.show', $value->id) }}"><i style="color : #4d94ff"
                                    class="fas fa-info"></i></a>
                            <a style="background-color : #fff ; border : #fff" class="btn btn-primary"
                                href="{{ route('roles.edit', $value->id) }}"><i style="color : #e6ac00"
                                    class="fas fa-edit"></i></a>
                            <button formaction="{{ route('roles.destroy', $value->id) }}"
