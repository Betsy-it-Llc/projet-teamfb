...
@section('content')
        ...
        <h2 class="text-xl telM:text-3xl font-sans font-bold text-black text-left relative 2xl:left-36">Projet</h2>
        ...
        <form action="{{ route('utilisateur.soutenirprojet') }}" method="GET">
            @csrf
            ...
            <div class="bg-transparent border border-gray-300 relative tel:w-72 telL:w-80 telM:w-96 md:w-96 tel:top-8 2xl:w-3/4 h-60 text-center rounded-sm">
                <div class="flex gap-3">
                    <div class="rounded-md w-32 h-32 tel:w-28 telM:w-32 relative mt-10 ml-5">
                        @if ($projet->getFirstMedia('header/'.$projet->description_courte))
                            <img style="height: 100%:width:100%" src="{{$projet->getFirstMedia('header/'.$projet->description_courte)->getUrl('miniature_projet')}}" alt="{{$projet->getFirstMedia('header/'.$projet->description_courte)->nom}}">
                        @else
