...
@section('content')
        ...
        <div class="grid grid-cols-1 grid-rows-2 gap-0.5 pl-5 relative">
            ...
            <p class="text-xl telM:text-3xl font-sans font-bold text-black text-left relative 2xl:left-36">Cause</p>
        </div>
        ...
        <form action="{{ route('utilisateur.searchcause') }}" method="GET">
            @csrf
            ...
            <div class="bg-transparent border border-gray-300 relative tel:w-72 telL:w-80 telM:w-96 md:w-96 tel:top-8 2xl:w-3/4 h-60 text-center rounded-sm">
                <div class="flex">
                    ...
                    <div class="relative right-3">
                        <div class="grid grid-cols-1 grid-rows-3 gap-1 relative left-3">
                            <p class="text-2xl tel:text-lg 2xl:text-lg font-sans font-bold text-black text-left relative 2xl:top-1 2xl:right-14">{{ $cause->nom_cagnotte }}</p>
                            <p class="text-lg font-sans font-bold text-black text-left relative">{{ $cause->description }}</p>
                            <div class="grid grid-cols-2 grid-rows-1 gap-1 relative right-5">
                                <p class="text-xl font-sans font-bold text-slate-800 text-right tel:text-xl lg:text-xl xl:text-xl 2xl:text-xl">{{ $cause->montant_actuel_cagnotte }}€</p>
                            ...
                            <div class="relative telM:top-2 2xl:top-5">
                                <p class="text-sm font-sans text-black text-left">Date: {{ $cause->date_creation->format('d m Y') }}</p>
                                <p class="text-sm font-sans text-black text-left">Type Projet: {{ $cause->types_Projet->valeur }}</p>
                            </div>
                        </div>
                ...
                <div class="grid grid-cols-2 grid-rows-1 gap-4 relative top-3">
                    ...
                    <button class="hover:bg-gray-400 bg-gray-300 rounded-md text-center w-32 tel:w-20 telL:w-20 telM:w-32 h-8 float-left relative tel:left-5 telM:bottom-2 2xl:left-2">
                        <a href="{{ route('utilisateur.recapsoutenir', ['id_projet' => $cause->id_projet]) }}" class="text-xs font-sans text-black">Soutenir</a>
                    </button>
                </div>
            </div>
