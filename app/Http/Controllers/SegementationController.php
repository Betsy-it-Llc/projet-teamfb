...

{
    ...
    public function index(Request $request)
    {
        ...
        $segementations = Segementation::sortable()->paginate($perPage);
        $value = session('id_segmentation');  // Stocker la variable dans la session de la table campagne
    ...
    public function multisearch_camp(Request $request)
    {
        ...
        $segementation = DB::connection('mysql')->table('postmeres')
            ->where('id_segmentation', 'LIKE', '%' . $multisearchid . '%')
            ->where('nom_segmentation', 'LIKE', '%' . $multisearchnom . '%')
            ->paginate(20);

        return view('segementations.index', ['segementations' => $segementation]);
    }
