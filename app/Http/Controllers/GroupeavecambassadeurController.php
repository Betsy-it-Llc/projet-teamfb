...
class GroupeavecambassadeurController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        ->paginate($perPage);
        return view('groupeavecambassadeurs.index',compact('groupeavecambassadeurs','perPage'))
    ...
    public function multisearch_amb(Request $request){
        ...
        ->leftJoin('ambassadeur_groupe','groupes.id_groupe','=','ambassadeur_groupe.id_groupe')
        ->leftJoin('utilisateur','ambassadeur_groupe.id_ambassadeur','=','utilisateur.id_utilisateur')
        ->select('groupes.id','groupes.id_groupe', 'groupes.nom as nom', 'groupes.nb_membres', 'groupes.type', 'groupes.lieu', 'groupes.statut_integration', 'groupes.tag_recherche', 'utilisateur.nom as utilisateur_nom', 'utilisateur.prenom')
        ->whereNotNull('utilisateur.id_ambassadeur')
        ->groupBy('groupes.nom')
        ...
        return view('groupeavecambassadeurs.index',['groupeavecambassadeurs'=>$groupeavecambassadeurs,'perPage'=>$perPage])
        ->with('i', (request()->input('page', 1) - 1) * 5);
    }
