...
class AmbassadeurController extends Controller
{
    ...
    public function groupeamb(Utilisateur $id_utilisateur, ambassadeurGroupe $ambassadeurGroupes, Ambassadeur $ambassadeur, Request $request,$id_ambassadeur)
    {
        
        /*$ida=0;
        $url = $request->fullUrl();
        $res=strlen($url);
        if($res==49){$ida = (int) substr($url,48, 1);}
        if($res==50){$ida = (int) substr($url,48, 2);}
        if($res==51){$ida = (int) substr($url,48, 3);}
        if($res==52){$ida = (int) substr($url,48, 4);}*/
        $ida=$id_ambassadeur;
        ...
        $ambassadeurGroupes = DB::connection('mysql')->table('groupes')
            ...
            ->where('ambassadeurs.id_ambassadeur', $ida)
            ->paginate(20);
